...
class PrestamosForm(forms.Form):
    cedula_usuario = IntegerField(label='Cedula del usuario', widget=NumberInput(
        attrs={'list': 'cedulas', 'autocomplete': 'off'}), min_value=1000000)
    nombre_elemento = ModelChoiceField(
        queryset=Inventario.objects.all(), label="Nombre elemento", empty_label="")
    cantidad = IntegerField(label="Cantidad", min_value=1, widget=NumberInput(attrs={'min': '1'}))
